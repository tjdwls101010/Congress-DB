...
def test_search_filters_can_use_trigram_indexes() -> None:
    ...
    assert "idx_utterances_content_trgm" in utterance_plan
...
def _delete_search_rows() -> None:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute("DELETE FROM utterances WHERE meeting_id = %s", (TEST_SEARCH_MEETING,))
        cur.execute("DELETE FROM meetings WHERE mnts_id = %s", (TEST_SEARCH_MEETING,))
...
def test_search_utterances_orders_by_similarity_and_returns_snippet() -> None:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO meetings (mnts_id, title, meeting_type, conf_date)
            VALUES (%s, '검색 테스트 회의', '상임위', '2026-01-03')
            """,
            (TEST_SEARCH_MEETING,),
        )
        cur.execute(
        ...
        cur.execute(
            """
            SELECT sequence, snippet, similarity_score
            FROM search_utterances('테스트전세사기', 10)
            WHERE meeting_id = %s
            """,
            (TEST_SEARCH_MEETING,),
        )
