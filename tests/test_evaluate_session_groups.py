...
def test_evaluate_label_rows_calculates_precision_and_recall() -> None:
    result = evaluate_label_rows(
        [
            {"label": "correct"},
            {"label": "correct", "notes": "codex-reviewed: looks correct"},
            {"label": "incorrect"},
            {"label": "missing"},
            {"label": ""},
        ]
...
def test_render_eval_report_marks_complete_agent_review_and_fallback(tmp_path) -> None:
    result = evaluate_label_rows(
        [
            {"meeting_type": "국정조사", "label": "correct", "notes": "codex-reviewed"},
            {"meeting_type": "국정조사", "label": "incorrect", "notes": "codex-reviewed"},
        ]
    )
    output = tmp_path / "SESSION-GROUP-EVAL.md"
    ...
    text = output.read_text()
    assert "Labeled review status: complete Codex-reviewed" in text
    assert "Agent-reviewed labels: 2" in text
    assert "Types requiring `utterances` sequence-window fallback: 국정조사" in text
