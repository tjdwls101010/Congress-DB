...
from .benchmark import DEFAULT_WORKER_LEVELS, BenchmarkResult, measure_workers
from .db import execute_many, get_conn
from .progress import ProgressReporter
from .scrape_minutes import UtteranceDraft, fetch_minutes, match_member, normalize_speaker_name, parse_minutes
...
def ingest_utterances(
    *,
    ...
    benchmark = measure_workers(
        lambda mnts_id, worker_count: _scrape_one_with_retry(
            int(mnts_id),
            retry_delays=retry_delays,
        ),
        items=target_meeting_ids[:benchmark_sample_size],
...
def _scrape_one(mnts_id: int) -> list[UtteranceDraft]:
    html, url = fetch_minutes(mnts_id)
    _, utterances = parse_minutes(html, mnts_id, url)
    if not utterances:
        raise RuntimeError(f"no utterances found for meeting {mnts_id}")
    return utterances
...
def _scrape_meetings(
    ...
    *,
    ...
    progress = ProgressReporter(label, len(meeting_ids))
    progress.start()
    with ThreadPoolExecutor(max_workers=worker_count) as pool:
        futures = {
            pool.submit(_scrape_one_with_retry, mnts_id, retry_delays=retry_delays): mnts_id
            for mnts_id in meeting_ids
        }
        for future in as_completed(futures):
            mnts_id = futures[future]
            try:
                scraped[mnts_id] = future.result()
                progress.advance()
