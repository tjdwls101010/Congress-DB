...
39. 시스템은 source별 cursor를 분리하되 fetch 범위 결정에는 쓰지 않는다. `bills`, `votes`, `meetings`, `utterances`, `session_groups` cursor는 각 source의 마지막 성공 시점을 관찰·감사하기 위한 `last_success_at` 기준점이다. *(개정 2026-06-04 — 이슈 #46.)*
