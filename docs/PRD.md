...
4. **API 카탈로그 1회성 문서화** — 277개 API의 작동/22대 데이터/사용 여부를 한 번 검증.
...
33. 시스템은 277개 API에 대한 1회성 검증을 수행하여 `api_catalog` 테이블에 작동 여부, 22대 데이터 보유 여부, 응답 row 수를 기록한다.
...
- **매일 277개 API 일괄 검증 스크립트** (PM 비판 반영, 1회성 카탈로그만)
