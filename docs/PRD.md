...
- **캘리브레이션 검증**: 초기 10% 적재는 산출물 목표가 아니라 전체 적재 전 병렬 수집 파라미터를 실측하는 단계다. 의원 286명 / 법안 1,700+건 / 회의 ~500건 / 표결 ~50,000건 / 발언 ~300,000건을 먼저 적재해 S1~S7 쿼리, rate limit, worker별 처리량, 오버헤드 증가 지점을 확인한 뒤 100% 적재로 확장한다.
