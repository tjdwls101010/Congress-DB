...
76. 시스템은 `bills.proposer`, `bills.committee`, `fetched_at`, `proc_result`, `cmt_proc_result`, `law_proc_dt`, `bill_source_aliases`, `utterances.id`를 이번 삭제 범위에서 제외한다. `bills.committee` 삭제는 `committee_id -> committee name` 정본 구조를 먼저 만든 뒤에만 재검토한다.
