...
64. 시스템은 '기타' 역할에 묻힌 정부측 actor(부처 실장·청장·○○위원장·정책관 등)를 `utterances.speaker_role` COMMENT의 함정 경고(정부측이 '기타'에 묻힘 → `speaker_title` 화이트리스트가 필요)로 알려, 소비자가 기존 `speaker_title_role_map`을 탐색해 회수하게 한다. **새 `speaker_affiliation` 테이블이나 `is_government_actor` 판정 컬럼은 두지 않는다** — '정부측인가'는 경합하는 판정이라 소비자 몫이고 raw `speaker_title`이 authoritative. `speaker_role` enum도 확장하지 않는다. 〔2026-06-11 소비-원칙 감사; 2026-06-12 가이드 정리로 함정을 COMMENT에 일원화(전용 레시피 제거)〕
