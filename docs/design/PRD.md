...
64. 시스템은 '기타' 역할에 묻힌 정부측 actor(부처 실장·청장·○○위원장·정책관 등)를 기존 `speaker_title_role_map` + DB-QUERY-GUIDE Q4 패턴을 **1급 가이드 레시피로 승격**해 회수 가능하게 한다. **새 `speaker_affiliation` 테이블이나 `is_government_actor` 판정 컬럼은 두지 않는다** — '정부측인가'는 경합하는 판정이라 소비자 몫이고 raw `speaker_title`이 authoritative. `speaker_role` enum도 확장하지 않는다. 〔2026-06-11 소비-원칙 감사〕
65. 시스템은 위원회 연결을 정규화하되 **`bills.committee_id`가 이미 존재**하므로(31개 값, 18,161/18,361행) 새 canonical 테이블을 만들지 않는다. 유일한 갭인 `meetings.comm_name`→기존 `committee_id` 매핑(대부분 공백 정규화, 24→30/31)을 얇은 `meeting_committee_map` 또는 검증된 JOIN 레시피로 채운다. membership(누가 어느 위원)은 명부 API(`nktulghcadyhmiqxi`) 전체·안정 회수를 검증하는 슬라이스가 통과한 뒤에만 적재하며, 검증 시 `member_committees` 제외 결정을 공식 재검토한다. 〔2026-06-11 소비-원칙 감사〕
