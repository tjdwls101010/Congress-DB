...
4. **"정부측 발언" ≠ `speaker_role IN ('국무위원(장관)','차관')`만.** 경찰청장직무대행·법원행정처장·금융위원장 등 정부측이 `'기타'`(263k)에 묻혀 있고, **'정부측'은 고정 필터가 아니라 질문별 판정**이다 — speaker_title 탐색으로 화이트리스트를 구성한다(§4 Q4).
...
### Q4. "정부측" 발언 — 장관·차관 + '기타'에 묻힌 정부측 actor
`speaker_role IN ('국무위원(장관)','차관')`만으로는 정부측의 일부만 잡힌다. `'기타'`(263k 발언)에 정부측이 대량 묻혀 있고(법원행정처장 4,367·경찰청장직무대행 2,995·금융위원장 2,677·고위공직자범죄수사처장 1,794 …), **"정부측"은 고정 필터가 아니라 질문 맥락에 따른 판정**이라 컬럼으로 두지 않았다. 접미사 LIKE로는 263k 중 88k(33%), 소속(부·청·처) 패턴을 더해도 154k(58%)만 잡히고 그나마 과·과소 포함이 섞인다(대통령비서실장·감사원장·합참의장은 구조 표지가 없어 빠지고, 노조 지부위원장 등이 끼어든다). 권위 축은 raw `speaker_title`이니 **질문에 맞는 직함을 audit 테이블에서 탐색해 화이트리스트를 구성**하라.
```sql
-- (a) 부처 장·차관 (enum으로 깔끔)
SELECT * FROM utterances WHERE speaker_role IN ('국무위원(장관)','차관');

-- (b) '기타'에서 질문 관련 정부측 직함을 빈도순으로 탐색 → 골라서 (a)에 UNION
SELECT speaker_title, n_utterances
FROM speaker_title_role_map
WHERE speaker_role = '기타'
  AND speaker_title ~ '(부|청|처|위원회|위원장|실장|국장|정책관|단장|총장|총재|비서실|감사원|참모|사령관)'
ORDER BY n_utterances DESC;     -- 출발점(기타의 69%)일 뿐 — 결과를 보고 질문에 맞게 가감
-- 고른 직함으로:  SELECT * FROM utterances
--                 WHERE speaker_title IN ('고위공직자범죄수사처장','경찰청장직무대행', …);
```
**org_type 판단 보조(컬럼 아님 — 직접 판정):**
- **명백 정부측** — 부처 장·차관·실/국장·정책관 · 청장 · 법원행정처(장/차장) · 고위공직자범죄수사처장 · 대통령비서실장 · 감사원(장/사무총장) · 군 수뇌(합참의장·각군총장·사령관) · 독립위원회 위원장(공정위·금융위·방통위·인권위)·한국은행총재·중앙선관위.
- **경계 모호 — 질문 맥락에 맞게 스킬이 포함 결정(보수적으로):** 인사청문 **후보자**(임명 전) · **지자체장**(시·도지사) · **공공기관·공기업 장**(공사 사장·공단 이사장) · **검찰**(검찰총장·검사장).
- **정부측 아님(高카운트 노이즈):** `반장`(4,431)·`진술인`(4,185) 등 절차·증언 역할(`n_mona=0`). 빈도 높다고 포함 말 것.

**시나리오 예(검증):**
- *전세사기 정부대응* → role(국토교통부 장·차관) + `'기타'`의 `국토교통부주택토지실장`(729)·`국토교통부전세사기피해지원단장`(88).
- *채상병 수사외압* → role(법무부 장·차관) + `'기타'`의 `고위공직자범죄수사처장`(1,794)·`경찰청장직무대행`(2,995)·`국방부장관직무대행`(1,125)·`해병대사령관`(101).
