...
## 2026-07-19 — 파이프라인·운영·신선도 개선 라운드 완료 (F1~F4, WI1·WI4·WI4b·WI5·WI6, migration 034·037·038)

라이브 조사에서 "에러 없이 조용히 틀리는" 실증 결함 4종이 확인돼(F1 공포·계보가 증분에 없어 스테일, F2 재의결 표결 소실, F3 러너 death로 적재 정지, F4 신선도 침묵), 스키마가 아니라 파이프라인·운영·신선도를 고쳤다. **결정·조치:** ① **F1/F1b(WI1)** — `bill_final_outcomes`를 증분·백필 스테이지로 편입하고 대상 선정을 `is_fully_resolved`로 교체(outcome 있어도 법률안이 공포 대기면 재조회, 공포 비대상 비-법률은 종착이라 제외 — 라이브 91건 겨냥·169 오포함 0). upsert는 비파괴 COALESCE. ② **F4(WI4)** — `data_freshness` 뷰(도메인별 last_ingest_at·latest_fact_date, `migration 034`)로 단정 전 기준일을 확인·병기하게 하고 `fetched_at` COMMENT를 소비 규율로 교체. ③ **구조 함정 졸업(WI4b)** — `is_law_bill`(`037`)·`prom_law_nm_norm`(`038`) 생성컬럼으로 "매번 재조립하던 기계적 규칙"을 엔진 계산으로 승격(razor: IMMUTABLE·drift 불가·저작 판단 없음만). ④ **F3(WI5)** — GitHub-호스트 신선도 워치독(`freshness-watchdog.yml`, 공개 congress_ro)으로 적재 정지·수집 누락을 빨간불로 표면화(러너 death로 예약 run이 조용히 cancelled되던 사각 차단). 러너 launchd 서비스화는 사용자 지시로 이번 라운드 유보. ⑤ **소비 규율(WI6)** — 부모 레포 congress 스킬에 신선도·재의결·원문링크 규율 + 함정 다이어트. **라이브 적용·검증(2026-07-19):** 5개 마이그레이션 Neon owner 적용(적용 전후 회귀팩 PASS + RLS 재활성 0), `make safe-update` 1회로 신규 스테이지 실행 — 무손상 PASS(백업 브랜치 자동 정리), 공포 +32·대안계보 +489행 추가. 결과 **F1 미공포갭 30→0**, 공포 이력 신선도 **06-10→07-19**, C1 대안계보 커버리지 461건 노출(아래 참조). 표적 헤드리스 e2e(V-A~V-D)로 스킬 발화 실증.
