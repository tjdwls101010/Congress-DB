...
```mermaid
erDiagram
    members ||--o{ bills : "rst_mona_cd (단일 대표발의 편의 FK)"
...
API `nwvrqwxyaytdsfvhu` (의원 인적사항)에서 받은 286명으로 시작한다. 다른 22대 API가 참조하지만 인적사항 API에 없는 MONA_CD는 FK와 JOIN 보존을 위해 최소 이름만 가진 stub row로 추가될 수 있다.
...
| `rst_mona_cd` | TEXT REFERENCES members(mona_cd) | 단일 대표발의일 때의 편의 FK. 복수 대표발의는 NULL 가능 |
...
**인덱스**: `rst_mona_cd`, `propose_dt`, `proc_result`, pg_trgm on (`bill_name`, `summary`).

---

### 3. `bill_lead_proposers` — 대표발의 N:M

국회 API의 `RST_MONA_CD`는 단일 값이 아닐 수 있다. 복수 대표발의자를 모두 보존하기 위해 정규화한다.

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `bill_id` | TEXT REFERENCES bills(bill_id) | |
| `mona_cd` | TEXT REFERENCES members(mona_cd) | |
| `order_no` | SMALLINT | API 원본의 순서 (`RST_MONA_CD` 순) |
| | | **PK(bill_id, mona_cd)** |

**인덱스**: `mona_cd` (의원별 대표발의 검색).
...
### 8. `meeting_bills` — 회의↔법안 N:M junction
...
**인덱스**: `meeting_id`, `speaker_mona_cd`, pg_trgm on `content`.

---

### 10. `session_groups` — Q&A 그룹
...
### 11. `api_catalog` — 사용 확정 OpenAPI 검증 결과
...
PRD의 "외부 API 사용 목록" 표에 있는 OpenAPI endpoint만 적재한다 (회의록 본문 HTML은 OpenAPI가 아니라 별도). 한 번 채워지면 거의 변하지 않는다 (재검증은 수동). 미사용 API의 메타는 `.Seongjin/legacy_congress/국회 api.db`에 보존 — [ADR 0001](adr/0001-api-catalog-scope.md) 참고.
