...
### 11. `api_catalog` — 사용 확정 10개 OpenAPI 검증 결과
...
PRD의 "외부 API 사용 목록" 표에 있는 10개 OpenAPI endpoint만 적재한다 (회의록 본문 HTML은 OpenAPI가 아니라 별도). 한 번 채워지면 거의 변하지 않는다 (재검증은 수동). 미사용 API의 메타는 `.Seongjin/legacy_congress/국회 api.db`에 보존 — [ADR 0001](adr/0001-api-catalog-scope.md) 참고.
