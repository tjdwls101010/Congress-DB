...
-- 안건 목록
SELECT order_no, sub_name, bill_id FROM agenda_items
WHERE meeting_id = ? ORDER BY order_no;
...
운영 모니터링은 카탈로그 매일 검증 스크립트가 아니라, **실제 사용 중인 API의 로드 실패 알림**으로 대체 (PM 1차 결정).
...
PM gate: `dead_letters=0`, session_group integrity error 0, sanity-check S1~S7 review 가능, data-completeness 잔여 공백 expected/accepted일 때 Supabase migration을 승인한다.
...
- "한 회의에서 안건 X에 대한 발언만" — 본문이 안건별 분해 안 됨 (PM 통찰: 안건은 메뉴판)
- "PDF/영상 원본" — URL만 보존, 다운로드 안 함
