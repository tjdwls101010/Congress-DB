...
- 발언↔특정 법안 **직접 귀속은 원천이 주지 않습니다.** 한 회의에 수십~수백 법안이 걸리므로(fanout 평균 32), "이 회의에서 다뤄짐"을 직접 증거로 단정하면 과잉입니다. `bill_meeting_contexts`의 `linked_bill_count`로 fanout을 보고 판단하세요.
- `meeting_bills`로 집계 시 `count(*)`는 fanout으로 부풀려집니다 → `count(DISTINCT meeting_id)`.
