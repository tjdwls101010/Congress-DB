...
**발언 (Utterance)**:
회의록 본문의 한 발언. 화자(speaker_name + speaker_title)와 시퀀스(meeting 내 순번)로 식별. 의원의 발언은 `speaker_mona_cd`로 의원과 join 가능. 단 매핑되는 건 **의원 직함 발언뿐**(의원 직함 한정 ~100%)이고, 전체 발언 기준 화자-의원 매핑률은 **~61.5%**다 — 나머지 ~38.5%는 장관·국무총리·차관·증인·참고인·전문위원 등 비-의원 화자로 `speaker_mona_cd`가 NULL이다. 비-의원측 조회는 아래 **발언 역할** 정규화로 보강 예정.
...
**발언 역할 (Speaker Role)** _(도입 예정 — M2)_:
