...
위원회 심사에서 여러 법안 내용을 통합해 새 **대안**을 만들고 원안들을 *대안반영폐기*할 때, 폐기된 원안과 그 내용을 흡수한 대안 법안 사이의 연결. `bill_relations`(absorbed_bill_id → alternative_bill_id)로 보존한다. 이 연결이 없으면 "이 주제가 과거에 어떻게 입법됐고 무엇이 법으로 남았나"를 추적할 수 없다(폐기 원안 3,676건, 통과 대안에 연결고리 없음).
